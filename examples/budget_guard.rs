/// Fixed-point budget path: CAS reserve → commit → conservation proof
///
/// ```bash
...
fn main() {
    ...
    engine.ensure_tenant("desk-alpha", 100_000_000 * MICROCENTS_PER_CENT);
    ...
    for i in 0..iterations {
        let (_, id) = engine.try_reserve("desk-alpha", 10_000);
        if let Some(id) = id {
    ...
    println!(
        ...
        engine.remaining_microcents("desk-alpha")
    );
}
