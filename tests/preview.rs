...
/// `Selection` holds a propensity in whole basis points, never below one. At
/// a 1 bp exploration rate over 22 near-best candidates an alternative's real
/// probability is a small fraction of a basis point, so the rounded weight
/// undercounts it about twentyfold. Only the exact fraction, carried from the
/// exploration record, weighs it correctly, through real outcomes.
#[test]
fn at_one_basis_point_over_22_candidates_only_the_exact_propensity_weighs_correctly() {
    ...
    let (x, record) = (0..5_000_000_u64)
        .map(|seq| {
            let x = input(seq);
            (x, explore(&logger, x, wide(1), &KEY).unwrap())
        })
        .find(|(_, r)| r.explored)
    ...
    let target_models: Vec<_> = twenty_two()
        .into_iter()
        .map(|mut m| {
